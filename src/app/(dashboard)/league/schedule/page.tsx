...
export default function LeagueSchedulePage() {
  ...
  const endpoint = `/api/user/league/schedule?q=${encodeURIComponent(query)}&status=${encodeURIComponent(statusFilter === "all" ? "" : statusFilter)}&page=${currentPage}&limit=${ITEMS_PER_PAGE}`;
  const { data, error, isLoading } = useApiSWR<LeagueScheduleResponse>(endpoint);

  const columns: Array<DataTableColumn<LeagueMatchScheduleItem>> = useMemo(
    () => [
      {
        key: "league",
        header: "League",
      ...
      },
      {
        ...
        render: (row) => (
          <div className="space-y-1">
            ...
            <Typography.Small className="text-primary-300">
              Round {row.round ?? "-"} {row.stage ? `• ${row.stage}` : ""}
            </Typography.Small>
          </div>
        ),
      },
      ...
      },
      {
        ...
        render: (row) => (
          <div className="space-y-1">
            ...
            <Typography.Small className="text-primary-300">
              {row.result ? row.result.resultStatus : "No result"}
            </Typography.Small>
          </div>
        ),
      },
    ...
    <div className="space-y-6">
      <div>
        ...
        <Typography.Paragraph className="text-primary-300 text-sm mt-1">
          Full schedule, roster players, and match results for your team.
        </Typography.Paragraph>
      </div>
      ...
      <DashboardSection title="Matches">
        <DashboardCard>
          ...
          {!isLoading && !error && data && data.items.length === 0 ? (
            <Typography.Paragraph className="text-primary-300">
              No league matches found for your team yet.
            </Typography.Paragraph>
          ) : null}
