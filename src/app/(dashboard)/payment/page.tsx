...
export default function UserPaymentPage() {
  ...
  const endpoint = `/api/user/payment?q=${encodeURIComponent(query)}&status=${encodeURIComponent(statusFilter === "all" ? "" : statusFilter)}&purposeType=${encodeURIComponent(purposeTypeFilter)}&provider=${encodeURIComponent(providerFilter)}&page=${currentPage}&limit=${ITEMS_PER_PAGE}`;

  const { data, error, isLoading } = useApiSWR<UserPaymentRecapResponse>(endpoint);
  const detailEndpoint = selectedInvoiceNumber
    ? `/api/user/payment/${encodeURIComponent(selectedInvoiceNumber)}`
    : null;
  const {
    data: detailData,
    error: detailError,
    isLoading: isLoadingDetail,
  } = useApiSWR<UserPaymentDetailResponse>(detailEndpoint);
  ...
  const paymentColumns: Array<DataTableColumn<UserPaymentRecapItem>> = useMemo(
    () => [
      {
        key: "invoice",
        header: "Invoice",
        render: (row) => (
          <div className="space-y-1">
            ...
            <Typography.Small className="text-primary-300">
              {formatDateTime(row.createdAt, { fallback: "-" })}
            </Typography.Small>
          </div>
        ),
      },
      ...
      },
      {
        ...
        render: (row) => (
          <div className="flex items-center justify-center gap-2">
            {row.status.trim().toLowerCase() === "pending" && row.checkoutUrl ? (
              <button
                type="button"
    ...
    <div className="space-y-6">
      <div>
        ...
        <Typography.Paragraph className="text-primary-300 text-sm mt-1">
          Check your payment history and status report.
        </Typography.Paragraph>
      </div>

      <DashboardSection title="Report">
          ...
          <div className="mb-4 grid grid-cols-1 gap-3 lg:grid-cols-12">
            ...
            <div className="lg:col-span-2">
              <Form.Text
                ...
                onChange={(event) => setPurposeTypeFilterInput(event.target.value)}
        ...
        {!isLoadingDetail && !detailError && detailData ? (
          <div className="space-y-2">
            <Typography.Paragraph className="text-primary-300 text-sm">
              Invoice:{" "}
              <span className="text-primary-100">{detailData.payment.invoiceNumber}</span>
            </Typography.Paragraph>
            <Typography.Paragraph className="text-primary-300 text-sm">
              Status: <span className="text-primary-100">{detailData.payment.status}</span>
            </Typography.Paragraph>
            <Typography.Paragraph className="text-primary-300 text-sm">
              Provider:{" "}
              <span className="text-primary-100">{detailData.payment.provider}</span>
            </Typography.Paragraph>
            <Typography.Paragraph className="text-primary-300 text-sm">
              Channel: <span className="text-primary-100">{detailData.payment.channel ?? "-"}</span>
            </Typography.Paragraph>
            <Typography.Paragraph className="text-primary-300 text-sm">
              Purpose:{" "}
              <span className="text-primary-100">{detailData.payment.purposeType}</span>
            </Typography.Paragraph>
            <Typography.Paragraph className="text-primary-300 text-sm">
              Purpose Ref:{" "}
              <span className="text-primary-100">{detailData.payment.purposeRef ?? "-"}</span>
            </Typography.Paragraph>
            <Typography.Paragraph className="text-primary-300 text-sm">
              Amount:{" "}
              <span className="text-primary-100">
                {formatAmount(detailData.payment.amount, detailData.payment.currency)}
              </span>
            </Typography.Paragraph>
            <Typography.Paragraph className="text-primary-300 text-sm">
              Created At:{" "}
              <span className="text-primary-100">
                {formatDateTime(detailData.payment.createdAt, { fallback: "-" })}
              </span>
            </Typography.Paragraph>
            <Typography.Paragraph className="text-primary-300 text-sm">
              Paid At:{" "}
            ...
            <Typography.Paragraph className="text-primary-300 text-sm">
              Expired At:{" "}
              <span className="text-primary-100">
                {formatDateTime(detailData.payment.expiredAt, { fallback: "-" })}
              </span>
            </Typography.Paragraph>
            <Typography.Paragraph className="text-primary-300 text-sm">
              Updated At:{" "}
              <span className="text-primary-100">
                {formatDateTime(detailData.payment.updatedAt, { fallback: "-" })}
              </span>
            </Typography.Paragraph>
          </div>
        ) : null}
