...
const mapPayerUsernames = async (accountIds: number[]) => {
  const validIds = Array.from(
    new Set(
      accountIds.filter((id) => Number.isInteger(id) && id > 0),
    ),
  );
  ...
  const accounts = await prisma.web_accounts.findMany({
  ...
  return new Map(
    accounts.map((account) => [account.id, account.user?.username ?? "-"]),
  );
};
...
const buildSummary = (
  ...
  } satisfies AdminPaymentRecapSummary;
};
...
export async function GET(request: Request) {
  try {
    ...
    const payerMap = await mapPayerUsernames(
      payments
        .map((payment) => payment.payer_account_id ?? 0)
        .filter((id) => id > 0),
    );
    ...
    return apiFromLegacy(
      {
        ...
        items: payments.map((payment) => ({
          ...
          payerUsername:
            payment.payer_account_id && payerMap.has(payment.payer_account_id)
              ? payerMap.get(payment.payer_account_id)
              : "-",
    ...
    logger.error("Admin payment recap fetch error:", error);
